...
## ROUND 40 — นโยบาย "ค่าเป็นคำ = ไม่ผ่าน" ทำให้ keep-best ทิ้งใบที่อ่านถูกทั้งใบ (2026-09-11)

commit `32507a9` — ปิดเคส 325-HK ที่ ROUND 39 เปิดค้างไว้ว่า "ตัวที่เจ็บที่สุดในคลัง"

### ต้นเหตุ

`6eceffa` เปลี่ยนแถวที่ใบเขียนผลเป็นคำ (`Traces`) จาก SKIP เป็น FAIL · `gridBeatsFlat` ตัดที่บรรทัดแรก
`if (grid.summary.fail > 0) return false` → candidate ที่ `parse-structural-grid` อ่านได้ **12 แถว (10 PASS)**
ถูกปฏิเสธทั้งก้อนเพราะมี `Traces` 2 แถว → คง flat ที่ได้ 3 แถว 0 PASS

⇒ **anti-regression rule ของ keep-best กับนโยบาย verdict ชนกันโดยตรง** ทุกใบที่มีคำปนในตารางที่ grid อ่านออก
จะเสีย PASS **ทั้งใบ** ไม่ใช่แค่แถวนั้น

### แก้อย่างไร (5 จุด · ลำดับตามที่รันจริง)

1. `coa-pipeline.ts` `numericFailCount(r)` = FAIL ที่ `result != null` — ใช้แทน `summary.fail` ใน `gridBeatsFlat`
   (ใช้ร่วมทั้ง grid / thai / HQ challenger). เหตุผล: FAIL ที่อ่านเลขได้แล้วหลุดเกณฑ์ = อาจจับคู่คอลัมน์ผิด
   ตัดทั้งใบถูกแล้ว · FAIL ที่เกิดเพราะใบเขียนคำ = สิ่งที่อยู่บนใบจริง ตัดแล้วแถวที่เหลือหายตาม
2. `parse-structural-grid.ts` `emitGridItems` — ช่องเกณฑ์ที่เป็นข้อความ (`淡黄色` `Absent` `K2Ti6O13`)
   `classifySpec` รับแต่เกณฑ์ตัวเลขเลยทิ้งไป → หยิบข้อความดิบมาเป็น `specRaw` (ข้ามช่องที่อ่านเป็นเลขได้)
3. ไฟล์ใหม่ `grid-text-rows.ts` `adoptGridTextRows` — แถวข้อความของใบที่มีเส้นตารางจริง เชื่อช่องของ grid
   ไม่เชื่อการจับคู่ของ LLM
4. `coa-pipeline.ts` — ข้าม `downgradeCopiedTextPasses` เมื่อ `isDeterministicGrid && gridSource === "structural"`
   (ช่องเกณฑ์กับช่องผลแยกกันอยู่แล้ว ไม่มีการคัดให้จับ)
5. `coa-evaluator.ts` — `needsReview: !!review` ตัดเงื่อนไข `specCellEquality && r !== 0`
   (ค่าผลเท่าเกณฑ์ที่อ่านจากช่องของตัวเองเป๊ะ → ผ่านทุกทิศที่เป็นไปได้ ธงบอกอะไรไม่ได้)

### ผลรัน — คลังรวม 40 ใบ

`_validate/_run-r46-corpus17-before.log` → `_run-r46-corpus17-after.log`
(★ ชื่อไฟล์หลอก — ทั้งคู่มี **40 ใบ** ไม่ใช่ 17 · คลังถูกรวมเป็นชุดเดียวระหว่างทาง ★)

| | before | after |
|---|---|---|
| รวม | 307P/9F/45S rows=361 nr=39 | **322P/11F/38S rows=371 nr=40** |

| ไฟล์ที่ขยับ | before | after | อ่านว่าอะไร |
|---|---|---|---|
| 325-HK | 0P/0F/3S | **10P/2F/0S** | เคสหลัก — กู้แถวที่ grid อ่านถูกอยู่แล้วกลับมา |
| Suzorite 325-HK | 0P/1F/2S | 5P/1F/0S | เหตุเดียวกัน |
| RI-015 | 9P/1F/4S | 11P/1F/2S | ชื่อแถวกู้ได้ `(udd)qd`→`Pb (ppm)` · `(udd) qs`→`Sb (ppm)` |
| TXAX-MA p1+p2 | 7P/0F/1S | 6P/0F/1S | **แก้ PASS ปลอม ไม่ใช่ regression** — เดิม LLM ตั้งชื่อ `結晶相` ผิดเป็น `色相` แล้ว PASS ด้วยการเทียบข้อความ |

อีก **46 จาก 51 บล็อกเหมือนเดิมเป๊ะ**

### ★ noise ของ gate — วัดได้ครั้งแรก ★

2 รันที่โค้ดเดียวกันเป๊ะ (`_run-r46-corpus17-after.log` vs `_run-r47-all40-after.log`) ต่างกัน **เฉพาะ `1F1710`**
(ใบ DuPont 5 block): p2 `3P` ↔ `12P` · p4 `14P/1S` ↔ `11P/0S` = ±6P · อีก **49/51 บล็อกตรงกันทุกตัว**

⇒ **delta ของ `1F1710` ห้ามสรุปว่าเกิดจาก code** · delta ที่ไฟล์อื่นเชื่อได้

### gate ที่รัน

`npx tsc -p .` = 0 · unit 9 suite: `spec-cell-equality` 14 · `keep-best-flagging` 41 · `parse-structural-grid` 53 ·
`evaluator` · `coa-pass-guard` · `coa-fail-guard` · `coa-grounding` · `result-normalizer` 40 · `spec-normalizer` 76

### กับดักของ baseline (เสียเวลาไป 1 รอบ)

`_run-r45-corpus17-before.log` ใช้เป็น before ไม่ได้ — 6 ไฟล์ตายด้วย
`pipeline FAILED: PDF_GRID_DOWN: pdfplumber จบด้วยรหัส 3221225794` (= `0xC0000142 STATUS_DLL_INIT_FAILED`,
python สตาร์ตไม่ขึ้นตอน RAM ตึง) ทำให้ SUMMARY ขาดไป 64 แถว

⇒ **ก่อนใช้ log ไหนเป็น baseline ต้อง `grep -c "pipeline FAILED"` = 0 และอ่านบรรทัดแรก `— N PDFs` เสมอ**

### ค้างไว้

- PAG-80 4 แถวไม่ผ่าน (เกณฑ์ merged cell คุมผลรวม) — user ยังไม่เคาะ
- `Traces` 4 แถว + `N/A` 1 แถว ไม่ผ่านตามนโยบาย — ทางปิดที่เสนอไว้คือให้ `Traces/ND/Nil` เทียบเกณฑ์
  **ขอบบนเท่านั้น** (`le`/`lt` หรือ `between` ที่ `min === 0`) = PASS ส่วน `N/A` คง FAIL · user ยังไม่เคาะ
- `1F1710` p4 `Fiber Length` อ่าน `1.090` เป็น `601` — HQ challenger อ่านถูกแต่แพ้ keep-best แล้วเลขนั้นไม่ถึงคน
- TAIHEIYO แถวหาย 1 (ROUND 39 open) = `[grounding] ตัด 1 row pH` ในรอบของ HQ ที่ชนะ — HQ สะกดชื่อไม่ตรง flat
  เลยถูกตัดว่าเป็นของที่ LLM แต่ง · แถวนั้นเป็น SKIP ไม่กระทบ verdict
- ใบไทยแบบสแกนยังไม่มีของจริงสักใบในคลัง (`[th-ocr]` ไม่เคยยิง)

---

## ROUND 41 — เกณฑ์ที่ใบพิมพ์ช่องเดียวจากสองช่อง อ่านทิศจากคอลัมน์แทนจากข้อความ (2026-09-11)

commit `b3169f7` — user เปิดใบ CIIR1066 แล้วชี้เองว่า "spec มีแต่ min ไม่มี max"

### ต้นเหตุ

ใบพิมพ์ `0.02  –  (ว่าง)` ใต้หัว `Specification` = เกณฑ์ขั้นต่ำ ไม่มีขอบบน · OCR ทิ้งขีดกลาง (ไม่มี token
ในช่วง x 1430–1600 เลยสักแถว) → ข้อความที่ LLM เห็นคือ
...
★ **แถวที่ขอบล่างหายจะให้ข้อความหน้าตาเหมือนกันเป๊ะกับแถวที่ขอบบนหาย** ★ — cell ว่างไม่เหลือร่องรอย
⇒ ตัดสินทิศจาก flat text ไม่ได้จริง ๆ ระบบจึงออก bare-eq → SKIP "ทิศหาย" (ถูกตามกติกา ห้ามเดา)

### แก้อย่างไร

`spec-pair-recovery.ts` เป็นเจ้าของเคสนี้อยู่แล้ว (โมดูลเจอ header `Avg.|Max.|Min.|Specification` ของใบนี้
และยิงกับแถว MOONEY อยู่ก่อนแล้ว) แต่บังคับ `nums.length === roles.length + 2` = ต้องมีเกณฑ์ครบ 2 ช่อง เลยถอย

เพิ่มเส้นทางแถวขอบเดียว โดยรับ `tokens?: OcrToken[]` เข้ามา (`coa-pipeline.ts` ส่งให้ · ข้ามเมื่อ
`isDeterministicGrid` เหมือนโมดูล token ตัวอื่น) แล้วอ่านทิศจากพิกัด:

- `findSpecColumns` — แถวที่พิมพ์ครบคู่บนใบเดียวกันสร้างคอลัมน์ต่ำ/สูง (median cx) · CIIR1066 ได้ **1386 / 1644**
- `sideOfSingleBound` — เลขตัวเดียวของแถวนั้นนั่งคอลัมน์ไหน · `0.02` ที่ cx 1386 = ช่องต่ำ → `specMin`

★ สัญญาณเป็นพิกัดที่ engine เดิม/LLM ปลอมไม่ได้ ไม่ใช่ verdict ★

**ถอยทั้งใบเมื่อ:** แถวครบคู่ < 2 แถว · แถวครบคู่ใดมีช่องซ้าย > ช่องขวา · เลขตัวเดียวตกกลางระหว่าง 2 คอลัมน์
(ห่าง < 0.3 เท่าของระยะคอลัมน์) · ไม่ตกคอลัมน์ไหนเลย (> 0.25 เท่า) · 2 คอลัมน์ห่างน้อยกว่า 2 เท่าความสูงตัวอักษร ·
ไม่มี token (หน้า text-layer)

### ผลรัน — คลังรวม 40 ใบ

`_run-r46-corpus17-after.log` → `_validate/_run-r49-all40-ciir.log`

| | ก่อน | หลัง |
|---|---|---|
| รวม | 322P/11F/38S rows=371 nr=40 | **333P/11F/35S rows=379 nr=38** |
| CIIR1066 p1 | 8P/0F/1S | **9P/0F/0S** |
| CIIR1066 p2 | 8P/0F/1S | **9P/0F/0S** |

`47/51 บล็อกเหมือนเดิม` · อีก 2 บล็อกที่ขยับคือ `1F1710` p2/p4 = **noise ที่วัดไว้แล้วใน ROUND 40**
(p2 ได้ 3P / 12P / 15P คนละรอบโดยโค้ดไม่เปลี่ยน)

★ พิสูจน์ว่าไม่ใช่ผลของ diff นี้ด้วย log เอง ★ — `grep "[spec-pair]"` ทั้งคลังยิงแค่ 2 ครั้ง ทั้งคู่เป็น
`ANTIOXIDANT(0.02→≥0.02)` ของ CIIR1066 ไม่มีไฟล์อื่นถูกแตะเลย

ผลพลอยได้: CIIR1066 ไม่มี SKIP เหลือ → HQ challenger ไม่ต้องยิง → 113.8s → 52.9s

### gate ที่รัน

`npx tsc -p .` = 0 · `spec-pair-recovery.test.ts` **16/16** (เพิ่ม 6 เคส: กู้ทิศช่องต่ำ · กู้ทิศช่องสูง ·
ตกกลาง 2 ช่อง → ไม่แตะ · แถวครบคู่แถวเดียว → ไม่แตะ · ซ้าย > ขวา → ไม่แตะ · ไม่มี token → ไม่แตะ)

### หมายเหตุเรื่องจอ

จอไม่ต้องแก้ — ตารางมีคอลัมน์ Min / Max แยกอยู่แล้ว แถวนี้ขึ้น `เกณฑ์ 0.02 · Min 0.02 · Max —`
คนอ่านเห็นว่าไม่มีขอบบน (คนละเรื่องกับ FC-250-1500 ที่ใบไม่มีคอลัมน์เกณฑ์เลยแต่จอโชว์ `เกณฑ์` = `ผล`)

### ค้างไว้ — ผลรีวิวใบสแกนรอบ 2026-09-11 (Fable, read-only, grade ใบจริง 8 ใบ 88 แถว)

**วัดได้:** PASS ที่อ่านผิดแต่บังเอิญเข้าเกณฑ์ = **0 ใน 88 แถว** · เทียบ default vs HQ บนแถว PASS 12 หน้า
ค่าผลไม่ตรงกัน **0 แถว** ⇒ ความเสี่ยงที่เหลือของใบสแกนอยู่ที่ชั้น**ตีความ/จับคู่ ไม่ใช่ตัวเลขที่ OCR อ่าน**

ทำได้และคุ้ม เรียงตามผลต่อความถูกต้อง:

1. ย้าย ground truth เข้า repo (`_validate/ground-truth.tsv`) + ให้ `verify-4b-only.ts` diff ต่อแถว +
   dump `debug.ocrText` ต่อหน้า — **ทุกข้อที่เหลือต้องมีตัวนี้ก่อนถึงพิสูจน์ผลได้** (ตอนนี้ gate นับแค่ P/F/S
   แถวที่อ่านผิดแต่เข้าเกณฑ์จึงมองไม่เห็นโดยโครงสร้าง)
2. PAG-80 merged spec cell = เกณฑ์ของ**ผลรวม** (11.9+16.8=28.7 ∈ 20~30 · 48.3+18.3=66.6 ∈ 60~70) →
   `shared-spec-cell.ts` ต้อง emit แถวรวม ไม่ใช่ก๊อปเกณฑ์ขึ้นแถวบน · สัญญาณ = ป้ายตะแกรงต่อกันเป็นลูกโซ่
   `-(a)+(b)` แล้ว `-(b)+(c)` · **รอ user เคาะ** (−4 FAIL)
3. `Traces/ND/Nil/Absent` เทียบเกณฑ์**ขอบบนเท่านั้น** (`le`/`lt` หรือ `between` ที่ `min === 0`) = PASS ·
   `N/A` คง FAIL (ไม่ได้วัด ≠ วัดแล้วต่ำกว่า) · แก้ที่ `coa-evaluator.ts` **ไม่ใช่** `result-normalizer`
   (map เป็น 0 = เสกเลขที่ใบไม่ได้พิมพ์ + ยุบ N/A กับ Traces เข้าด้วยกัน) · **รอ user เคาะ** (−4 FAIL)
4. ทิศเกณฑ์ที่ LLM ใส่เองบนเลขเปล่าต้องมี operator ใน OCR line ถึงเชื่อ (`spec-recovery.ts`) —
   PR1950W_4063 `Residue on sieve(1mm)` ได้ `ge 0` = ค่าเท่าไรก็ผ่าน
5. ค่าตรงขอบช่วง + OCR มี range token ตรงตัว (`0.45 to0.50`) → PASS + ธง (Kemolit `BulkDensity`, RI-015
   `Particle Size`) = ตัดสินได้ +2
6. `flagChallengerPasses` pool ต้องรวมแถว FAIL ของตัวเดิมด้วย ไม่งั้น challenger ที่ PASS ด้วยเลขคนละตัว
   ขึ้นเขียวล้วน · และ HQ ควรยิงเมื่อมี FAIL ปักธง digit-scramble (`1F1710` default อ่าน `601` HQ อ่าน `1.09`
   แต่เลขนั้นไม่ถึงคน)

ที่**ไม่ควรทำ** พร้อมเหตุผลที่ตรวจซ้ำแล้ว:

- overlap column assign ของ `scanned-grid-builder` — path นี้ **ไม่เคยยิงเลยตั้งแต่ r42**
  (`grep -c "deterministic scanned-vector"` = 0 ทุก log) แก้ไปวัดผลไม่ได้
- Kemolit `Retention on 60 mesh <0.0` — ใบพิมพ์ `<0.0` ตรงตัว OCR 2 ไฟล์อ่านตรงกัน SKIP ถูกแล้ว
- `Foreign Particles` — `method-cell-shift` **ยิงจริงทุกรอบ** ใบพิมพ์ Test Method ว่าง / Spec `VISUAL` /
  Result `Absent` จริง → ไม่ใช่บั๊กโมดูล ต้องขยายข้อ 3 แทน
- ปิด cls ของ RapidOCR (RI-015 หัวคอลัมน์กลับหัว) — 1 ใบใน 22 แลกกับหน้าที่ป้อนกลับ 180° จริงพังทั้งแผ่น
- `pdf_table.py` เก็บ newline ในช่อง (TXAX-MA) — text-layer ไม่ใช่ใบสแกน · blast radius = structural grid
  ทุกหน้า เพื่อ 2 แถว SKIP ที่ซื่อสัตย์
- regex CJK ใน `coa-grounding.ts` — **แก้ไปแล้ว** (`coa-grounding.ts:20`) บันทึกเก่าที่ว่ายังค้างนั้นตกยุค
- อ่านซ้ำด้วยเครื่องที่ 2 เพื่อยืนยัน PASS — วัดแล้ว 12 หน้า ค่าต่างกัน 0 แถว ไม่มีสัญญาณให้ซื้อ
- ใช้ตรา `JUDGEMEMT:PASS` / `ACCEPT By: QA Dept.` เป็นสัญญาณ — เปลี่ยน verdict ไม่ได้ตามนโยบาย และเป็นตรา
  ของ QA ฝั่งที่ระบบนี้กำลังจะแทน = วงกลม

**ยังพิสูจน์ไม่ได้:** ใบไทยแบบสแกนไม่มีของจริงสักใบ (`[th-ocr]` = 0 ทุก log) → ขอใบจริง 3–5 ใบจากหน้างานก่อน
**อย่าเขียน code ก่อนมีใบ**
