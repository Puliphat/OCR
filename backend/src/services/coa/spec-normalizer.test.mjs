import { normalizeSpec, normalizeSpecFromCandidate } from "./spec-normalizer.js";
...
for (const [input, expected] of cases) {
  ...
  if (expected === null) ok = got === null;
    ...
    ok = got !== null &&
      got.op === expected.op &&
      (expected.min === undefined || Math.abs(got.min - expected.min) < 1e-9) &&
      (expected.max === undefined || Math.abs(got.max - expected.max) < 1e-9) &&
      (expected.value === undefined || Math.abs(got.value - expected.value) < 1e-9);
  }
  if (ok) { pass++; }
  else { fail++; console.log(`FAIL  "${input}"  expected=${JSON.stringify(expected)}  got=${JSON.stringify(got)}`); }
}

// candidate (separate Min/Max columns)
const candCases = [
...
for (const [input, expected] of candCases) {
  ...
  const ok = got !== null &&
    ...
    (expected.value === undefined || Math.abs(got.value - expected.value) < 1e-9);
  if (ok) pass++;
  else { fail++; console.log(`FAIL CAND  ${JSON.stringify(input)}  expected=${JSON.stringify(expected)}  got=${JSON.stringify(got)}`); }
}
