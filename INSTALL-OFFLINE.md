...
| pip ตายตอน `--find-links`: `Could not find a version that satisfies the requirement` | พิมพ์ path ของ `wheels` หรือ `frozen.txt` ผิด (ชุดคำสั่งยาว ตกตัวอักษร) | copy คำสั่งข้อ 3.3 ไปทั้งบรรทัด อย่าพิมพ์เอง · เช็ค `Test-Path C:\coa-setup\ocr-offline\wheels` ต้อง True |
